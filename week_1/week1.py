...
class HtmlDiagramFactory:

    def make_diagram(self, width, height):
        return HtmlDiagram(width, height)
    
    def make_rectangle(self, x, y, width, height, fill="white",
                       stroke="black"):
        return HtmlRectangle(x, y, width, height, fill, stroke)
    
    def make_text(self, x, y, text, fontsize=12):
        return HtmlText(x, y, text, fontsize)

class HtmlDiagram:

    def __init__(self, width, height):
        pxwidth = width * HTML_SCALE
        pxheight = height * HTML_SCALE
        self.diagram = [HTML_START.format(**locals())]
        outline = HtmlRectangle(0, 0, width, height, "lightgreen", "black")
...
class HtmlRectangle:
    ...
    def __init__(self, x, y, width, height, fill, stroke):
        ...
        width *= HTML_SCALE
        height *= HTML_SCALE
        self.html = HTML_RECTANGLE.format(**locals())
...
class HtmlText:
    ...
    def __init__(self, x, y, text, fontsize):
        x *= HTML_SCALE
        y *= HTML_SCALE
        fontsize *= HTML_SCALE // 10
        self.html = HTML_TEXT.format(**locals())
